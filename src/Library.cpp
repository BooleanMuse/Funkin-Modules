...
`path::string()` on Windows converts to the machine's local code page, which
mangles any mod folder with a character outside it — and mod folders are named by
people, in every language there is. `u8string()` is the one that does not. */
...
void Walk(const fs::path& root, Harvest* h, int maxDepth = 8, size_t maxFiles = 60000) {
	std::error_code ec;
	...
	for (; it != end; it.increment(ec)) {
		...
		// Symlinks are not followed: a mod folder with a link back up to the home
		// directory would otherwise take the walk with it.
		if (it->is_symlink())
			continue;
		...
		Harvest::File f;
		f.path = PathText(it->path());
		f.lower = Slashed(f.path);
		f.stem = Lower(PathText(it->path().stem()));
		std::string ext = Lower(PathText(it->path().extension()));
	...
	if (h->sheets.empty() && !pngs.empty()) {
		for (Harvest::File& p : pngs) {
			for (const std::string& x : xmls) {
				if (Lower(PathText(fs::path(x).stem())) == p.stem) {
					h->sheets.push_back(p);
					h->xmlOf.push_back(PathText(fs::path(p.path).parent_path()) + "/" +
					                   PathText(fs::path(x).filename()));
					break;
				}
			}
		}
...
std::string ModNameFor(const fs::path& root, const std::string& file) {
	std::error_code ec;
	fs::path rel = fs::relative(fs::path(file), root, ec);
	if (ec || rel.empty())
		return PathText(root.filename());
	auto it = rel.begin();
	if (it == rel.end())
		return PathText(root.filename());
	std::string first = PathText(*it);
	// The file sits directly in the root: the root is the mod.
	if (++it == rel.end())
		return PathText(root.filename());
	// "mods/MyMod/..." and "assets/..." — a folder that is plainly plumbing is
	// not a mod name, so go one deeper.
	std::string low = Lower(first);
	if (low == "mods" || low == "assets" || low == "content") {
		if (it != rel.end()) {
			std::string second = PathText(*it);
			if (++it != rel.end())
				return second;
		}
	}
...
void Library::scan(const std::vector<std::string>& folders) {
	...
	for (const std::string& folder : folders) {
		...
		for (const Harvest::File& jf : h.jsons) {
			...
			CharacterRef ref;
			ref.name = PathText(fs::path(jf.path).stem());
		...
		for (size_t i = 0; i < h.sheets.size(); i++) {
			if (claimed[i])
				continue;
			CharacterRef ref;
			ref.name = PathText(fs::path(h.sheets[i].path).stem());
